...
export function createMySQLAdapter<A extends Record<string, any>>(db: MySqlDatabase<any, any, any>, table: MySQLSessionTable): Adapter<A> {
	return new MySQLCoreAdapter<A>(db, table);
}

class MySQLCoreAdapter<A extends Record<string, any>> implements Adapter<A> {
