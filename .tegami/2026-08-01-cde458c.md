...
- Added a loud-failure check so a session `secretHash` stored as TEXT (instead of a binary column) surfaces as an `AdapterError` instead of silently breaking validation
